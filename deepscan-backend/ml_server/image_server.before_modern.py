...
# FaceForensics++, published video-level AUROC 96.6% on Celeb-DF-v2. Measured
# here on held-out face crops (calibration_gend.json): AUC 0.85 vs 0.756 for our
# 128px Xception, and 5.8% false positives on real faces at its 0.5 boundary vs
# 20% at the Xception operating point. Preferred face-manipulation detector.
    ...
    try:
        ...
        if hf_vit_model is not None:
            try:
                ...
                raw = logits.flatten().float().tolist()
                p_ai = float(torch.sigmoid(logits.flatten()[0]).item())
        ...
        p_deepfake = None
        if has_face and gend_model is not None and face_crop is not None:
            try:
...
async def predict_video_as_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        if hf_vit_model is not None:
            try:
                frame_ai_probs = [run_hf_vit_inference(preprocess_rgb_array_for_hf_vit(rgb)) for rgb, _ in frames]
