...
def decide(
    ...
    base = _decide_forensic(p_ai, p_deepfake, face_detected, synthid, calibration, detector_errors)
    return _provenance_adjust(base, metadata, c2pa)
...
def _decide_forensic(
    ...
    # 5. REAL requires POSITIVE evidence of authenticity, not merely the absence
    #    of an alarm. "No detector fired" is not proof; a detector that cannot
    #    certify (null *_low) contributes nothing toward REAL, and its silence
    #    must not be counted as a clean bill of health.
    #
    #    Specifically: if a face is present, whole-image synthetic detection
    #    does NOT cover the face-swap threat model — measured on real face
    #    swaps the AI detector reads ~0.01 (see trace_pipeline.py). So for a
    #    face image, an AI-detector-clean reading alone is a single weak signal
    #    against a threat it cannot see, and cannot certify REAL by itself.
